...
if __name__ == "__main__":
    ...
    for i in range(a, b):
        ...
        # # 对 test\\NORMAL 里的数据进行增强
        # batch_enhance_image(
        #     class_input_dir=os.path.join(root_path, 'Chest_XRay\\test\\NORMAL'),
        #     class_output_dir=os.path.join(root_path, 'data_enhance\\test\\NORMAL'),
        #     suffix=i)
        # # 对 test\\PNEUMONIA 里的数据进行增强
        # batch_enhance_image(
        #     class_input_dir=os.path.join(root_path, 'Chest_XRay\\test\\PNEUMONIA'),
        #     class_output_dir=os.path.join(root_path, 'data_enhance\\test\\PNEUMONIA'),
        #     suffix=i)
        # print('test NORMAL:', normal_num * (i + 1))
        # print('test PNEUMONIA', pneumonia_num * (i + 1))
        # print('test Total:', num * (i + 1))
        # print('-' * 50)

        # 对 val\\NORMAL 里的数据进行增强
        batch_enhance_image(
            class_input_dir=os.path.join(root_path, 'Chest_XRay\\val\\NORMAL'),
            class_output_dir=os.path.join(root_path, 'data_enhance\\val\\NORMAL'),
            suffix=i)
        # 对 val\\PNEUMONIA 里的数据进行增强
        batch_enhance_image(
            class_input_dir=os.path.join(root_path, 'Chest_XRay\\val\\PNEUMONIA'),
            class_output_dir=os.path.join(root_path, 'data_enhance\\val\\PNEUMONIA'),
            suffix=i)
        print('val NORMAL:', normal_num * (i + 1))
        print('val PNEUMONIA', pneumonia_num * (i + 1))
        print('val Total:', num * (i + 1))
        print('-' * 50)